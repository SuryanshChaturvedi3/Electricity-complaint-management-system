...
function StudentLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("token", res.data.token); // Store token for API calls
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
      <form
        ...
        className="w-full max-w-md bg-slate-900 p-8 rounded-2xl shadow-2xl border border-slate-800 space-y-6 transition-all duration-300 hover:border-slate-700"
      >
        <div className="text-center space-y-2">
          {/* UPDATED: Clickable Icon to go back to Landing Page */}
          <button 
            type="button"
            onClick={() => navigate("/")} 
        ...
        {/* UPDATED: Removed onClick, handled by form onSubmit */}
        <button
          type="submit"
