...
function ComplaintDetail() {
  ...
  async function handleResolve() {
    try {
      const res = await api.put(`/technician/complaints/${id}/resolve`);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={() => navigate("/tech/complaints")}
        ...
        <div
          className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border ${
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <InfoItem
              icon={<MapPin size={16} />}
              label="Location"
              value={`${complaint.hostelname}, Room ${complaint.roomnumber}`}
            />
            <InfoItem
              icon={<User size={16} />}
              label="Student Roll"
              value={complaint.rollnumber}
            />
            <InfoItem
              icon={<Tag size={16} />}
        ...
        <div className="p-8 bg-zinc-950/50 border-t border-zinc-800">
          {complaint.approvalStatus !== "resolved" ? (
            <button
              ...
              className="w-full md:w-auto px-8 py-4 bg-amber-500 hover:bg-amber-400 text-black font-black rounded-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg"
            >
              <CheckCircle size={20} />
      ...
      <div>
        ...
        <p
          className={`text-sm font-semibold ${
            isHighlighted ? "text-amber-500" : "text-zinc-200"
          }`}
        >
          {value}
        </p>
